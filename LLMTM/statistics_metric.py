...
def analyze_graph_features(edges):
    """
    Analyze a series of general structural and cognitive complexity metrics of the graph.
    ...
    Args:
        edges (list): List of edges, each element in the form (u, v, t, op).
    
    Returns:
        dict: Dictionary containing various general features of the graph.
    """
    ...
    # Extract node and edge information from active edges to construct the graph
...
def statistic():
    motif_name = ["triangle","4-tailedtriangle"]
    for motif in motif_name:
        result_dict = []
        # Use project-relative logs path
        base_path = f"logs/example/run_one_task/judge_contain_motif/api/{motif}"
        ...
        for M_folder in M_folders:
            ...
            flag_folder = os.path.join(M_folder_path, f"results_{motif}_gpt-4o-mini_cotFalse_roleFalse_k0_dyg1_edge0_imp0_short0_temperature0.0_maxtokens8192_motifname{motif}_motif1_change0_use_agent0_api1.json")
            with open(flag_folder, "r") as f:
                data = json.load(f) 
                flag_data_api = data["error_folders"]
                flag_data_api += data["failure_folders"]
            ...
            output_dir = M_folder_path
            json_path = os.path.join(output_dir, f"result_dict_{motif}_sigle_test.json")
            ...
            csv_path = os.path.join(output_dir, f"result_dict_{motif}_sigle_test.csv")
