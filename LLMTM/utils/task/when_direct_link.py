...
class DyGraphTaskWhenDirectLink(DyGraphTask):
    """
    Task: Ask for the time when two given nodes are first directly connected, and after that, when they are first disconnected.
    Corresponds to original scripts: When.py, QA.py (Operation), evaluate.py (Operation)
    """
    def generate_qa(self, info, *args, **kwargs):
        """
        Generates QA pairs. Ported from QA.py (Operation).
        Selects an edge that has at least one add and one delete operation, finds the first add and first delete time.
        """
        ...
        for item in context_orig:
            try:
                ...
                print(f"Warning (WhenDirectLink - QA): Skipping unparseable row: {item} - {e}")
                continue
            except Exception as e:
                print(f"Warning (WhenDirectLink - QA): Error processing row: {item} - {e}")
                continue

        if not context_typed:
            print("Error (WhenDirectLink - QA): No valid context data.")
            # In this case, cannot generate valid QA, can return None or raise error
            # Return None to let the caller (e.g., in runner.py gen) know to skip this instance
            return None

        # Sort by time
        context_typed.sort(key=lambda x: x[2])

        # Find all edges with at least one 'a' and one 'd' operation
        ...
        if not eligible_edges:
            ...
            # Likewise, cannot generate valid QA
            return None

        # Randomly select one eligible edge
        selected_edge = random.choice(eligible_edges)
        u_query, v_query = selected_edge # Extract the node pair for the query
        ...
        # Find first 'd' (must occur after the first 'a')
        if first_add_time != -1:
             for t, op in operations:
        ...
        if first_add_time == -1 or first_delete_time == -1:
             print(f"Warning (WhenDirectLink - QA): Edge {selected_edge} did not have a valid first add/delete pair.")
             return None # Cannot generate valid QA

        # The answer is (first_add_time, first_delete_time)
        answer = (first_add_time, first_delete_time)

        # context for the prompt is the original unsorted, untyped list
        qa = {
            "context": context_orig, # Use original format to pass to prompt
    ...
    def generate_prompt_examplars(self, num, *args, **kwargs):
        # Ported from When.py, ensure format is correct
        qa_examples_raw = [
            (
...
4. Final Output: Present first_connected_time and first_disconnected_time.""", # COT
        ...
        # Convert format
        qa_formatted = []
        for c, q, s, a in qa_examples_raw:  # Fix: now correctly unpacking 4 elements
             # Answer needs special handling, as make_qa_example might not directly support tuple answer formatting
             # We format it as a "t1, t2" string
             answer_str = f"{a[0]}, {a[1]}"
             qa_formatted.append([list(c), list(q), s, answer_str])  # Include CoT element s


        # Modify the base class's make_qa_example to support this custom answer string
        # Or build the example string directly here
        if num == 0:
            return ""
        examples = []
        for c, q, s, a_str in qa_formatted[:num]:  # Fix: correctly unpack 4 elements
             context_prompt = self.generate_context_prompt(c) # Use base class method to format context
             question_prompt = self.generate_prompt_question(q) # Use this class's method to format question
             if self.args.add_cot == 1:
                 example = f"{context_prompt}{question_prompt}\n{s}\nAnswer: {a_str}"
             else:
                 example = f"{context_prompt}{question_prompt} Answer: {a_str}" # Directly append the answer string
    ...
    def evaluate(self, qa, response, use_agent):
        """
        Evaluates the model response. Ported from evaluate.py (Operation).
        Parses the two integers at the end of the response and compares with the answer tuple.
        Returns:
            metric: 1 (correct), 0 (wrong but correct format), -1 (cannot parse format)
            extracted_answer: The parsed (t1, t2) tuple or None
        """
        ...
        if answer_start_index != -1 or use_agent == 1:
            ...
            # Try matching format: "The nodes are first connected at time \\boxed{2} and then disconnected at time \\boxed{3}."
            ...
            # Try matching format: "First connected at time \\boxed{3}, first disconnected at time \\boxed{4}."
            ...
            # Try matching format: "**First Connected:** \\boxed{2}\n- **First Disconnected:** \\boxed{4}"
            # Specifically match this format, handling various whitespace characters
            ...
            # More precise match, handling whitespace after colon and possible newlines
            ...
            # Try matching format: "**First Connected:** \\boxed{2}\n- **First Disconnected:** \\boxed{4}" (More general match)
            ...
            # Try matching format: "First connected at t=1, first disconnected at t=3."
            ...
            # Try matching format: "First connected at **t=1**, first disconnected at **t=2**"
            ...
            # Try matching format: "First connected at time: 0  \nFirst disconnected at time: 2"
            ...
            # Try matching format: "First Connection: 2  \nFirst Disconnection: 3"
            ...
            # Try matching format: "First Connection Time: 3  \nFirst Disconnection Time: 4"  
            ...
            # Try matching format: "First Connection: 3\n- First Disconnect: 4"
            ...
            # Try matching format: "The nodes first connect at time `3` and then disconnect at time `4`."
            ...
            # Try matching format: "The nodes first connect at time 0 and disconnect at time 4."
            ...
            # Try matching format: "The nodes are first connected at timestamp 0 and then disconnected at timestamp 4."
            ...
            # Try matching format: "The nodes are first connected at timestamp 2 and first disconnected at timestamp 4."
            ...
            # Try matching format: "The nodes are first connected at time 2 and then disconnected at time 3."
            ...
            # Try matching format: "- First connected at timestamp 3.\n- First disconnected at timestamp 4."
            ...
            # Try matching format: "First Connected: 4  \nFirst Disconnected: 4"
            ...
            # Try matching format: "First connected at time `t = 0`, first disconnected at time `t = 1`."
            ...
            # Try matching format: "Connect Time: 1, Disconnect Time: 3"
            ...
            # Try matching format: "First connected at 3, first disconnected at 4"
            ...
            # Try matching format: any text containing two boxed numbers, regardless of content between
            ...
            # Try matching format: "\\boxed{3}, \\boxed{4}" or "\boxed{3}, \boxed{4}"
            ...
            # Try matching format: "First connected at time 0, first disconnected at time 4."
            ...
            # Try matching format: "First connected at time: 1, first disconnected at time: 5"
            ...
            # Try matching format: "First connected at time `3`, first disconnected at time `4`"
            ...
            # Try matching format: "Answer: 3, 3" (Numbers directly at the start of the extracted part)
            # Check if the extracted part starts with digits
            ...
            # Try matching various answer formats
            patterns = [
            ...
            if use_agent == 1:
                ...
                try:
                    ...
                    return -1, None
            # If none of the above patterns matched, try the most basic number matching
            ...
            if match:
                ...
                # If two numbers in the correct format cannot be found
                return -1, None # Format error
        else:
