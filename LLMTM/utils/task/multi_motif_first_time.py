...
#                 [],
#                 """**Chain of Thought:** # 1. My goal is to determine the first appearing time of all temporal motifs present in the given undirected dynamic graph.
...
4. Therefore, the first appearing time of all temporal motifs are: (triangle, 2), (3-star, 3), (4-path, 3), (4-cycle, 6), (4-tailedtriangle, 5), (butterfly, 6), (4-chordalcycle, 6), (bitriangle, 9).""",
    ...
    def evaluate(self, qa, response):
        ...
        """
        ground_truth = qa.get("answer", []) # Get the true answer
        extracted_answer = None
        metric = -1 # Default to parsing failure
        print("ground_truth: ", ground_truth)

        # Find the last list-formatted answer
        # First, try to find lists in code blocks
        code_blocks = re.findall(r'```(?:python)?\s*\n?(.*?)\n?```', response, re.DOTALL)
        if code_blocks:
            # Use the last code block
            answer_text = code_blocks[-1].strip()
        else:
            # If no code block, find content after the last "Answer:" marker
        ...
        if answer_text:
            try:
                ...
                # Clean the text, remove irrelevant markers
                ...
                if answer_text.strip().startswith('[') and answer_text.strip().endswith(']'):
                    try:
                        # Convert the string to a valid Python list format
                        # Replace single quotes with double quotes, ensure string format is correct
                        formatted_text = answer_text.replace("'", '"')
                        print("Formatted text for eval: ", formatted_text)
                        # Try to parse directly as a Python object
                        parsed_list = eval(formatted_text)
                        print("Successfully parsed list: ", parsed_list)
                        if isinstance(parsed_list, list):
                            for item in parsed_list:
                                # Modify condition, accept both list and tuple types
                        ...
                        # If eval fails, fall back to other parsing methods
                        pass

                # If Python list parsing failed, try other formats
                ...
                # Convert extracted answer and true answer to sets for comparison (ignore order)
                if not extracted_answer:  # If no answer was extracted
                    print("No answer could be extracted")
                    metric = 0
                else:
                    # Convert answers to dictionaries with motif name as key
                    extracted_dict = {}
