...
def setup_langchain_debug_logging():
    ...
    print(f"📝 LangChain debug输出将保存到: {debug_filename}")
    print(f"📝 原始stdout和stderr已重定向")

    return debug_filename

# 先定义debug_dir，再调用设置函数

debug_dir = None  # 初始化为None，后续在get_response中设置
debug_file = None  # 初始化为None，后续在get_response中设置
...
class AgentManager:
    ...
    def _load_llm(self):
        ...
        llm = ChatOpenAI(
            ...
            base_url=openai.base_url,         # 使用全局设置的值，但通过参数传递
            default_headers=openai.default_headers, # 使用全局设置的值，但通过参数传递
            stop=['\nObservation']
        )
        return llm
...
# - No need for specific node names and time points for the motif.

# - Use the GENERAL format in question for temporal motif definition.

# - Use the GENERAL motif definition format from the question directly in your Action Input - NEVER ask for clarification about the specific values; you MUST proceed using the general format.

# CRITICAL RULES (Most Important)

# - Firstly, CHECK if an Observation exists. If it does, MUST IMMEDIATELY output Final Answer.

# - The Final Answer MUST be the EXACT content from the last Observatioan, without any modification or conversational filler.

# Thought: Based on Observation, I now know the final answer, and **MUST** output the following Final Answer.

# Final Answer: EXACT content from the last Observatioan.

# CRITICAL RULES (Most Important)

# - Firstly, CHECK if an Observation exists. If it does, **MUST** output Final Answer.

# - The Final Answer MUST be the EXACT content from the last Observatioan, without any modification or conversational filler.

#Thought: you should always think about what to do and **MUST** output the following Action and Action Input.

# CRITICAL RULES (MOST IMPORTANT)

# - You **MUST** remember output Action, Action Input and Final Answer.
...
Thought:{agent_scratchpad}"""
    ...
    def _setup_logging(self, log_dir):
        ...
        class TeeOutput:
            ...
            def write(self, message):
                ...
                self.log.write(message)
                self.log.flush()
                # 仅在缓冲区中累积[llm/end]消息进行后续处理
                if '[llm/end]' in message:
                    self.buffer += message
                    if message.strip().endswith('}'):  # 确保JSON完整
            ...
            #     self.log.write(message)
            #     self.log.flush()
            #     # 不再需要 self.buffer 和 _process_buffer
        ...
        print(f"📝 LangChain debug输出将保存到: {log_filename}")
        print(f"📊 Agent统计信息将保存到: {stats_filename}")
    ...
    def get_response(self, task, prompt: str, agent_log_path: str, clear_history: bool = True):
        ...
        try:
            ...
            if response_content:
                ...
                print(f"🎟️ Tokens used: {self.tee_output.stats if self.tee_output else 'N/A'}")
                return {
    ...
    # def get_response(self, task, prompt: str, agent_log_path: str, clear_history: bool = True):
    #         """
    #         Gets a response from the Agent, now with token counting.
    #         """
    #         start_time = time.time()
    #         log_file = self._setup_logging(agent_log_path)
    #         prompt = extract_question(task, prompt)
    ...
    #             # --- 更新你的日志统计 (可选但推荐) ---
    #             # 你的 TeeOutput 统计逻辑可以简化
    #             # 你现在可以直接把 cb 的数据塞进去
    #             if hasattr(self.tee_output, "update_stats"):
    #                 step_info = {
    #                     "timestamp": datetime.datetime.now().isoformat(),
    #                     "execution_time": execution_time,
    #                     "tokens": token_stats # 把token信息也加到步骤里
    #                 }
    #                 self.tee_output.update_stats(
    #                     time=execution_time,
    #                     step_info=step_info
    ...
    #             # 你也可以直接更新 TeeOutput 的主 "stats" 字典
    #             if self.tee_output:
    #                 self.tee_output.stats.update(token_stats)
    #                 self.tee_output._save_stats() # 强制保存
    ...
    #                 print(f"⏱️ Execution time: {execution_time:.2f}s")
    #                 # 打印从 cb 获取的 token 统计
    #                 print(f"🎟️ Tokens used: {token_stats}") 
    #                 return {
    #                     "content": response_content,
    #                     "success": True,
    #                     "info": "Single attempt completed.",
    #                     "time": execution_time,
    #                     "tokens": token_stats  # 在返回结果中包含 token 统计
    #                 }
    #             else:
    #                 # ... (处理空响应)
    #                 print(f"❌ Query completed, but no content returned from Agent.")
    ...
    #                     "info": "Single attempt completed.",
    #                     "time": execution_time,
    #                     "tokens": token_stats # 即使失败也返回token
    #                 }
