...
# --- MODIFICATION 2: 添加 Tiktoken 回调处理器 ---
# 这个类用于在*客户端*计算 token
class TiktokenCallbackHandler(BaseCallbackHandler):
    """
    一个自定义的回调处理器，使用 tiktoken 在客户端估算 Token。
    这会捕获 Agent 执行过程中的 *所有* LLM 调用。
    """
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        try:
            # 估算编码。 "cl100k_base" 适用于 gpt-4, gpt-3.5-turbo
            self.encoding = tiktoken.get_encoding("cl100k_base")
            print(f"Token 估算器已加载: cl100k_base")
        except Exception:
            self.encoding = tiktoken.get_encoding("p50k_base")
            print(f"Token 估算器已加载 (回退): p50k_base")
    ...
    def on_llm_start(
        ...
        """在 LLM 调用开始时，计算 Prompt 的 Tokens。"""
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        """在 LLM 调用结束时，计算 Completion 的 Tokens。"""
    ...
    def stats(self) -> Dict[str, int]:
        ...
        # 返回与你 TeeOutput.stats 结构*完全一致*的字典
        return {
...
Thought:{agent_scratchpad}"""
    ...
    def get_response(self, task, prompt: str, agent_log_path: str, clear_history: bool = True):
        ...
        log_file = self._setup_logging(agent_log_path) # TeeOutput 在这里被创建
        ...
        try:
            ...
            # --- 2. 传入 config 运行 Agent ---
            result = self.agent_executor.invoke(inputs, config=config)
            
            # --- 3. 从回调中获取*本次调用*的统计 ---
            # (这会返回一个字典: {"total_tokens": ..., "prompt_tokens": ..., "completion_tokens": ...})
            token_stats = token_callback.stats

            execution_time = time.time() - start_time
            
            # --- 4. 更新 TeeOutput.stats ---
            step_info = {
                "timestamp": datetime.datetime.now().isoformat(),
                "execution_time": execution_time,
                "tokens": token_stats # (可选) 在 "steps" 里也记录一下
            }
            if hasattr(self.tee_output, "update_stats"):
                # 将 tiktoken 的统计结果 (token_stats) 传入
                # update_stats 会将其合并到 self.tee_output.stats 中
                self.tee_output.update_stats(
                    time=execution_time,
                    step_info=step_info,
                    token_stats=token_stats # ** 注入 token 统计 **
                )
            ...
            if response_content:
                ...
                # 现在 self.tee_output.stats 包含了 tiktoken 的估算值
                ...
                return {
                    ...
                    "info": "Single attempt completed.",
                    "time": execution_time,
                    "tokens": self.tee_output.stats if self.tee_output else {} # 即使失败也返回
                }
            ...
            return {
                ...
                "time": execution_time,
                "tokens": self.tee_output.stats if self.tee_output else {} # 即使异常也返回
            }
