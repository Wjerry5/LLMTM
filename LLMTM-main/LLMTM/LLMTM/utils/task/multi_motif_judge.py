...
def check_temporal_constraints(mapping, sorted_motif_events, context_times_all, motif_time_window):
    """
    Check if there exists a timestamp sequence for the given isomorphism mapping that satisfies temporal and window constraints.
    (Use iterative method to find valid sequences)
    """
...
def judge(context, motif_definition, motif_time_window):
    ...
    satisfying topological, temporal order and time window constraints (considering all timestamps).
    """
    ...
    # 5. If all mappings do not satisfy temporal constraints
    return "No"
...
class DyGraphTaskJudgeMultiMotif(DyGraphTask):
    ...
    def generate_prompt_examplars(self, num, *args, **kwargs):
        ...
        # The [ is red here because there is a bracket pairing error in the line below:
        # (7, 19, 1, 'a'), 10, 13, 1, 'a'), (10, 18, 1, 'a'),
        # After (7, 19, 1, 'a'), it should be (10, 13, 1, 'a'), not 10, 13, 1, 'a')
        # Correct format shown below, bracket pairing fixed:
...
4. Therefore, the graph contains the following temporal motifs: triangle, 3-star, 4-path, 4-cycle, 4-tailedtriangle, butterfly, 4-chordalcycle, bitriangle.""",
    ...
    def evaluate(self, qa, response, use_agent):
        """
        Evaluate whether the model response matches the true answer and compare.
        ...
        Returns:
            tuple: (matched_count, total_ground_truth_count)
                   matched_count: Number of matches with ground_truth
                   total_ground_truth_count: Total number of ground_truth items
        """
        # Fix: Correctly handle list type answers
        ground_truth_raw = qa.get("answer", [])
        if isinstance(ground_truth_raw, list):
            # If answer is a list, convert to lowercase set
            ground_truth_set = set(motif.strip().lower() for motif in ground_truth_raw if motif)
        elif isinstance(ground_truth_raw, str):
            # If answer is a string, split by comma and convert to lowercase set
        ...
        total_ground_truth_count = len(ground_truth_set)  # Total number of true answers
        ...
        try:
            ...
            if answer_start_index != -1 or use_agent == 1:
                ...
                # Split answer text by comma or space into list and remove whitespace
                extracted_motifs = set()
                for motif in re.split('[,\s]+', answer_text):
                    # Clean motif string: remove parentheses, quotes, dots and other unwanted characters
