...
def visualize_graph(n, temporal_edges, graph_path):
    ...
    # Automatically calculate subplot grid size
    cols = math.ceil(math.sqrt(num_snapshots))
    rows = math.ceil(num_snapshots / cols)

    fig, axes = plt.subplots(rows, cols, figsize=(4 * cols, 4 * rows))
    if rows * cols == 1:  # Handle case with single snapshot
        axes = np.array([axes])
    # Ensure axes is always iterable, even if it's a single Axes object initially
    axes = axes.flatten()

    # Create unified layout positions - use complete graph for layout consistency
    pos = nx.spring_layout(nx.complete_graph(n), seed=42)  # Use fixed seed

    # Current set of edges (normalized: tuple(sorted((u, v))))
    ...
    for t in range(timestamps[-1] + 1): # Iterate through all possible times up to max
        ...
        # If current time t is a snapshot point we need to draw
    ...
    try:
        ...
        print(f"Error (visualize_graph): Failed to save image {graph_path}: {e}")
    finally:
...
def create_colored_animation(n, temporal_edges, gif_path):
    ...
    all_edges_normalized = set()     # Record all edges that have appeared
    edge_last_op = {}            # Record last operation for each edge ('a' or 'd')
    edge_first_add_time = {}     # Record first addition timestamp for each edge

    fig, ax = plt.subplots(figsize=(7, 6))

    # Use same layout method as static graph
    ...
    # Store edge drawing objects for alpha updates
    ...
    def update(frame_idx):
        ...
        for edge in all_edges_normalized:
            ...
            first_add = edge_first_add_time.get(edge, t) # First addition time

            color = 'gray' # Default color
            width = 1.5    # Default width
            alpha = 0.2    # Default alpha (historical trace)

            if edge in current_edges_normalized: # If edge currently exists
                color = 'green'
                width = 2.5
                alpha = 1.0
            elif last_op == 'd': # If edge was last deleted
                color = 'red'
                width = 2.0
                # Alpha decays with time
                dt = t - first_add # Time since first addition
                # (Can adjust decay logic, e.g., based on last deletion time)
    ...
    try:
        ...
        print(f"Error (create_colored_animation): Failed to save GIF {gif_path}: {e}")
        # Try saving with a different writer if pillow fails
        try:
            print("Attempting to save GIF using 'imagemagick'...")
            ...
            print(f"Error (create_colored_animation): Failed to save GIF using 'imagemagick' as well: {e2}")
    finally:
